...
public class AsynchTwitter extends AsyncTask<Void, Void, List<twitter4j.Status>>
{
	...
	protected TwitterHandler twitterHandler;
	...
	private int maxTweetCount;
	private Twitter twitter;

	public AsynchTwitter(TwitterHandler twitterHandler)
	{
		this.twitterHandler = twitterHandler;

		this.drawableProfileImage = twitterHandler.drawableProfileImage;
		this.userID = twitterHandler.userID;
		this.userProfileImageURL = twitterHandler.userProfileImageURL;
		this.sinceUserTimelineID = twitterHandler.sinceUserTimelineID;
		this.sinceMentionsTimelineID = twitterHandler.sinceMentionsTimelineID;
		this.timelineSize = twitterHandler.timelineSize;
		this.maxImageCount = twitterHandler.maxImageCount;
		this.maxTweetCount = twitterHandler.maxTweetCount;
		this.twitter = twitterHandler.twitter;
	}
	...
	private List<twitter4j.Status> getUserTimeline()
	{
		...
		try
		{
			...
			// newer tweets
			if (result.size() > 0)
				twitterHandler.sinceUserTimelineID = result.get(0).getId();
		}
		catch (TwitterException e)
		{
			result = null;
	...
	private List<twitter4j.Status> getMentionsTimeline()
	{
		...
		try
		{
			...
			// newer tweets
			if (result.size() > 0)
				twitterHandler.sinceMentionsTimelineID = result.get(0).getId();
	...
	{
		twitterHandler.processFinish(result);
	}
}